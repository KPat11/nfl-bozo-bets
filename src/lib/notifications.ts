...
export async function sendPaymentReminder(userId: string, userName: string, week: number): Promise<void> {
...
export async function sendWeeklyReminder(userId: string, userName: string, week: number): Promise<void> {
  const user = await prisma.user.findUnique({
    where: { id: userId }
  })
...
export async function sendPaymentReminders(week: number, season: number): Promise<void> {
  ...
  for (const bet of unpaidBets) {
    await sendPaymentReminder(bet.userId, bet.user.name, week)
  }
}
